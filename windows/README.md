...
msstore 소스에서 `0x8a15005e`(server certificate did not match) 오류가 나면
winget 인증서 피닝 버그이므로, 관리자 PowerShell 에서 우회 후 재시도한다:

```powershell
winget settings --enable BypassCertificatePinningForMicrosoftStore
winget install --id 9pfxxshc64h3 --source msstore --accept-package-agreements --accept-source-agreements
```
